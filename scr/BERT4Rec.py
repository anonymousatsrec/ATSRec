...
from modules import Encoder, LayerNorm, BPRLoss, TransformerEncoder
...
class BERT4RecModel(nn.Module):
    def __init__(self, args):
        ...
        self.item_embeddings = nn.Embedding(self.n_items, self.hidden_size, padding_idx=0)  # mask token add 1
        ...
        self.apply(self._init_weights_BERT4Rec)

    def _init_weights_BERT4Rec(self, module):
    ...
    def reconstruct_train_data(self, item_seq):#(item_seq,target_pos, target_neg)
        """
    ...
    def calculate_loss(self, input_ids,target_pos, target_neg): # (input_ids,target_pos, target_neg)
        #item_seq = interaction[self.ITEM_SEQ]
        # masked_item_seq, pos_items, neg_items, masked_index = self.reconstruct_train_data(item_seq)
        item_seq = input_ids
        masked_item_seq, pos_items, neg_items, masked_index = self.reconstruct_train_data(item_seq)

        # pos_items = target_pos
        # neg_items = target_neg
        ...
        if self.loss_type == 'BPR':
            pos_items_emb = self.item_embeddings(pos_items)  # [B mask_len H]
            neg_items_emb = self.item_embeddings(neg_items)  # [B mask_len H]
    ...
    def full_sort_predict(self, interaction):
        ...
        # item_seq_len = interaction[self.ITEM_SEQ_LEN]
        # item_seq = self.reconstruct_test_data(item_seq, item_seq_len)
        item_seq=interaction
        seq_output = self.forward(item_seq)
        #seq_output = self.gather_indexes(seq_output, item_seq_len-1)  # [B H]
        test_items_emb = self.item_embeddings.weight #[:self.n_items]  # delete masked token
